...
class Flasher (BaseFlasher):
  ...
  kw = {}
  flash_addr=0
  def __init__ (self, spi=-1, cs=None, dbg=None, **kw):
     if type (spi) is int:
       spi = SPI(spi, **kw)
  ...
  def start (self):
    ...
    for i in range (5):
      self.WRDI()
      v = self.RDSR()
      if (not self.dbg) and v & 2: raise Exception ("Device not connected")
      self.WREN()
      v = self.RDSR()
      if (not self.dbg) and not (v & 2): raise Exception ("Device not connected")
    ...
    if v == 0 or v==0xff:
      ...
      v=rv[3]
      if (v == 0 or v==0xff) and not self.dbg:
